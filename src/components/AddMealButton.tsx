          ...
          <div className="bg-white w-full max-w-md mx-auto rounded-t-3xl p-6 animate-slide-up">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-4">
              <div>
                ...
                <input name="title" type="text" required placeholder="Sunday Dinner 🍝" className="famli-input" />
              </div>
              <div>
                <label className="famli-label">Date & Time</label>
                <input name="scheduledAt" type="datetime-local" required defaultValue={defaultDateTime} className="famli-input" />
              </div>
              <div>
                <label className="famli-label">Location <span className="text-stone-400 font-normal">(optional)</span></label>
                <input name="location" type="text" placeholder="Home, Restaurant name..." className="famli-input" />
              </div>
              <div>
                <label className="famli-label">Notes <span className="text-stone-400 font-normal">(optional)</span></label>
                <textarea name="notes" rows={2} placeholder="What's on the menu? Any requests?" className="famli-input resize-none" />
              </div>

              {error && <p className="text-red-500 text-sm">{error}</p>}

              <button type="submit" disabled={saving} className="famli-btn-primary w-full flex items-center justify-center gap-2">
                {saving ? <span className="w-5 h-5 border-2 border-white border-t-transparent rounded-full animate-spin" /> : "Plan this meal 🍽️"}
              </button>
            </form>
          </div>
        </div>
