    ...
    <div className="w-full max-w-md bg-white rounded-t-3xl p-6 animate-slide-up h-[85dvh] overflow-y-auto overscroll-contain touch-pan-y">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 pb-24">
        ...
        <div>
          ...
          <textarea name="notes" rows={4} placeholder="What's on the menu? Any requests?" className="famli-input resize-none" />
        </div>

        {error && <p className="text-red-500 text-sm">{error}</p>}

        <button type="submit" disabled={saving} className="famli-btn-primary w-full flex items-center justify-center gap-2 sticky bottom-0">
